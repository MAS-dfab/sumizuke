				...
				if time.ticks_diff(time.ticks_ms(), start) > 2000:
					if pinvalue(free_pin):
						...
						if parallel_click:
							...
							long_click_tcp = 1
							await client.publish(MODE_TOPIC, json.dumps("RESET_ALIGNMENT"), qos = 0)
							write_text_center("alignit!")
							await asyncio.sleep_ms(2000)
							align_number = 0
			if long_click_tcp == 0:
				if old_text != None:
					await client.publish(TCP_TOPIC, json.dumps("MarkerID:" f"{old_text}"), qos = 0)
				...
				if old_text != None:
					await client.publish(TCP_TOPIC, json.dumps(old_text), qos = 0)
					write_text_center("ID sent !")
					await asyncio.sleep_ms(2000)
					align_number += 1
					if align_number == 3:
